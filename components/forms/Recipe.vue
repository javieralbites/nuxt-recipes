          ...
          <v-container>
            <v-row>
              ...
              <v-col cols="4">
                <v-text-field
                  dense
                  label="Duración de la receta (minutos)"
                  outlined
              ...
              <v-col cols="4">
                <v-text-field
                  dense
                  label="Cantidad de platos de la receta"
                  outlined
              ...
              <v-col cols="12">
                ...
                <div
                  v-for="(ingredients, i) in recipe.ingredients"
                  :key="i + 'ingrediente'"
                >
                  <v-text-field
                    dense
...
export default {
  ...
  methods: {
    ...
    async onsubmit() {
      ...
      // console.log('my id is '+ this.$auth.user.id);
      // console.log('my token is '+this.$auth.strategy.token.get());
      ...
      await this.$apollo
        ...
        .then((data) => {
          ...
          this.$store.dispatch('snackbars/setSnack', {text:"Receta guardada", color:'success'})
        })
        .catch((e) => {
          console.log('error: '+e);
          this.$store.dispatch('snackbars/setSnack', {text:"Hay un error, intenta mas tarde", color:'error'})
